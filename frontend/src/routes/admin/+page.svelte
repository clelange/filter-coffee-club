<script lang="ts">
  ...
  import type { AppSettings, Dripper, FlavorTag, Grinder, Preset, PresetInput, Profile } from '$lib/types';
  ...
  let presetForm: PresetInput = $state({name:'',ratio:16,temperature_min_c:92,temperature_max_c:96,active:true,sort_order:0,grinder_ranges:[]});
  ...
  async function run(action:()=>Promise<unknown>, success:string):Promise<boolean> { error='';message='';try{await action();message=success;await load();return true;}catch(caught){error=caught instanceof Error?caught.message:'The change could not be saved.';return false;} }
  ...
  async function savePerson(person:Profile){const pin=pinResets[person.id]||undefined;await run(()=>api(`/people/${person.id}`,{method:'PUT',body:jsonBody({display_name:person.display_name,role:person.role,pin_change_required:person.pin_change_required,...(pin?{pin}:{})})}),`${person.display_name} updated.`);pinResets[person.id]='';}
  ...
  async function addPreset(event:SubmitEvent){event.preventDefault();if(await run(()=>api('/presets',{method:'POST',body:jsonBody(presetForm)}),'Preset added.'))resetPresetForm();}
  ...
  function grinderForRange(grinderId:number){return grinders.find((item)=>item.id===Number(grinderId));}
  function rangeStep(grinderId:number){const grinder=grinderForRange(grinderId);return isClickUnit(grinder?.setting_unit??'')?1:(grinder?.setting_step??0.1);}
  function rangeInputMode(grinderId:number){return isClickUnit(grinderForRange(grinderId)?.setting_unit??'')?'numeric':'decimal';}
  function addPresetRange(){const grinder=grinders.find((item)=>!presetForm.grinder_ranges.some((range)=>range.grinder_id===item.id));if(!grinder)return;presetForm.grinder_ranges=[...presetForm.grinder_ranges,{grinder_id:grinder.id,setting_min:grinder.soft_min??0,setting_max:grinder.soft_max??50}];}
  function removePresetRange(index:number){presetForm.grinder_ranges=presetForm.grinder_ranges.filter((_,itemIndex)=>itemIndex!==index);}
  function resetPresetForm(){presetForm={name:'',ratio:16,temperature_min_c:92,temperature_max_c:96,active:true,sort_order:Math.max(-1,...presets.map((preset)=>preset.sort_order))+1,grinder_ranges:[]};addPresetRange();}
...
{#if activeTab==='people'}
  <div class="admin-grid"><form class="panel" onsubmit={addPerson}><h2>Add a member</h2><label>New member display name<input bind:value={personForm.display_name} required /></label><label>Four-digit PIN<input bind:value={personForm.pin} inputmode="numeric" autocomplete="new-password" pattern="[0-9][0-9][0-9][0-9]" maxlength="4" required /></label><p class="hint">New accounts must replace this temporary PIN after their first sign-in.</p><label>Role<select bind:value={personForm.role}><option value="member">Member</option><option value="admin">Administrator</option></select></label><button class="primary">Add member</button></form>
    <section class="panel"><h2>Profiles</h2><div class="item-list">{#each people as person}<article><input aria-label="Display name" bind:value={person.display_name} /><select aria-label={`Role for ${person.display_name}`} bind:value={person.role}><option value="member">Member</option><option value="admin">Administrator</option></select><input aria-label={`New PIN for ${person.display_name}`} bind:value={pinResets[person.id]} inputmode="numeric" autocomplete="new-password" pattern="[0-9][0-9][0-9][0-9]" maxlength="4" placeholder="New PIN" /><label class="check pin-required"><input type="checkbox" bind:checked={person.pin_change_required} /> Require PIN change for {person.display_name}</label><button class="secondary" onclick={()=>savePerson(person)}>Save</button><button class="secondary" onclick={() => togglePerson(person)}>{person.active?'Deactivate':'Activate'}</button></article>{/each}</div></section></div>
...
{:else if activeTab==='presets'}
  <div class="stack"><form class="panel preset-creator" onsubmit={addPreset}><h2>Add recipe preset</h2><div class="preset-create-fields"><label>Name<input bind:value={presetForm.name} required /></label><label>Ratio<input type="number" bind:value={presetForm.ratio} min="1.1" max="30" step="0.1" required /></label><label>Min °C<input type="number" bind:value={presetForm.temperature_min_c} min="50" max="100" required /></label><label>Max °C<input type="number" bind:value={presetForm.temperature_max_c} min="50" max="100" required /></label><label>Order<input type="number" bind:value={presetForm.sort_order} /></label><label class="check"><input type="checkbox" bind:checked={presetForm.active} /> Active</label></div><div class="preset-ranges"><div class="section-heading"><h3>Grinder ranges</h3><button class="secondary" type="button" onclick={addPresetRange} disabled={presetForm.grinder_ranges.length>=grinders.length}>+ Grinder range</button></div>{#each presetForm.grinder_ranges as range,index}<div class="preset-range"><label>Grinder<select bind:value={range.grinder_id}>{#each grinders as grinder}<option value={grinder.id} disabled={presetForm.grinder_ranges.some((item,itemIndex)=>itemIndex!==index&&item.grinder_id===grinder.id)}>{grinder.manufacturer} {grinder.model}</option>{/each}</select></label><label>Minimum setting<input type="number" bind:value={range.setting_min} step={rangeStep(range.grinder_id)} inputmode={rangeInputMode(range.grinder_id)} required /></label><label>Maximum setting<input type="number" bind:value={range.setting_max} step={rangeStep(range.grinder_id)} inputmode={rangeInputMode(range.grinder_id)} required /></label><button class="secondary" type="button" onclick={()=>removePresetRange(index)}>Remove</button></div>{/each}</div><button class="primary">Add preset</button></form><section class="panel"><h2>FCC starting points</h2><div class="preset-list">{#each presets as preset}<article><input bind:value={preset.name} aria-label="Preset name" /><label>Ratio<input type="number" bind:value={preset.ratio} step="0.1" /></label><label>Min °C<input type="number" bind:value={preset.temperature_min_c} /></label><label>Max °C<input type="number" bind:value={preset.temperature_max_c} /></label>{#each preset.grinder_ranges as range}<label>Min clicks<input type="number" bind:value={range.setting_min} step="1" /></label><label>Max clicks<input type="number" bind:value={range.setting_max} step="1" /></label>{/each}<label>Order<input type="number" bind:value={preset.sort_order} /></label><label class="check"><input type="checkbox" bind:checked={preset.active} /> Active</label><button class="secondary" onclick={() => savePreset(preset)}>Save</button></article>{/each}</div></section>
...
<style>
  ...
  .item-list,.rack>div { display:grid; gap:7px; }.item-list article {display:grid;grid-template-columns:minmax(130px,1fr) 130px 110px 150px auto auto;gap:7px;align-items:center;padding:10px 0;border-bottom:1px solid var(--line)}.pin-required{min-height:auto;font-size:.78rem}.rack article { display:flex; justify-content:space-between; align-items:center; gap:10px; padding:10px 0; border-bottom:1px solid var(--line); }.rack { display:grid; grid-template-columns:repeat(3,1fr); gap:20px; }.rack span { padding:8px 0; }
  ...
  .preset-creator{display:grid;gap:18px}.preset-create-fields{display:grid;grid-template-columns:minmax(180px,2fr) repeat(4,minmax(80px,1fr)) auto;gap:10px;align-items:end}.preset-ranges{display:grid;gap:10px}.preset-ranges h3{margin:0}.preset-range{display:grid;grid-template-columns:minmax(180px,1.5fr) 1fr 1fr auto;gap:10px;align-items:end;padding:12px;border:1px solid var(--line);border-radius:13px}
