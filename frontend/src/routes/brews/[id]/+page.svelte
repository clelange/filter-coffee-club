<script lang="ts">
  ...
  async function load() {
    ...
    } catch (caught) { error = caught instanceof Error ? caught.message : 'Could not load this brew.'; }
  }
  ...
  async function finalize() {
    ...
    try {
      brew = await api<Brew>(`/brews/${brew.id}/finalize`, {
        method: 'POST', body: jsonBody({ water_g: actualWater, total_brew_time_s: finalMinutes * 60 + finalSeconds })
      });
      finishing = false;
      await wakeLock?.release();
      const session = await ensureSession();
      if (session?.device_mode === 'kiosk') await logout();
    } catch (caught) { error = caught instanceof Error ? caught.message : 'Could not finalize the brew.'; }
  }
...
{:else if brew.status === 'draft'}
  <section class="brew-mode">
    <div class="brew-heading">
      <div><p class="eyebrow">Brew mode · settings locked on screen</p><h1>{brew.coffee_name}</h1><p class="lede">{brew.coffee_roaster} · operator {brew.operator_name}</p></div>
      <span class="status draft">draft #{brew.id}</span>
    </div>
    <div class="recipe-display">
      <article class="hero-metric"><strong>{brew.dose_g}<i>g</i></strong><span>coffee dose</span></article>
      <div class="arrow">→</div>
      <article class="hero-metric"><strong>{brew.water_g}<i>g</i></strong><span>total water</span></article>
      <article class="recipe-cell"><strong>1:{brew.ratio}</strong><span>ratio</span></article>
      <article class="recipe-cell"><strong>{brew.temperature_c} °C</strong><span>water</span></article>
      <article class="recipe-cell"><strong>{brew.grinder_setting}</strong><span>{brew.grinder_name} · {brew.grinder_unit}</span></article>
      <article class="recipe-cell"><strong>{brew.target_flow_g_s ?? '—'} {brew.target_flow_g_s ? 'g/s' : ''}</strong><span>target pour rate</span></article>
      <article class="recipe-cell"><strong>{brew.servings}</strong><span>serving{brew.servings === 1 ? '' : 's'}</span></article>
      <article class="recipe-cell"><strong>{brew.dripper_name ?? '—'}</strong><span>{brew.filter_name ?? 'dripper / filter'}</span></article>
    </div>
    ...
    <div class="modal-backdrop" role="presentation">
      <div class="modal panel" role="dialog" aria-modal="true" aria-labelledby="finish-title" tabindex="-1">
        ...
        <div class="field-grid">
          ...
          <label>Actual water g<input type="number" bind:value={actualWater} min="1" max="5000" inputmode="numeric" /></label>
        </div>
        <div class="actions"><button class="primary" onclick={finalize} disabled={finalMinutes * 60 + finalSeconds <= 0}>Finalize and invite tasters</button><button class="secondary" onclick={() => (finishing = false)}>Back</button></div>
      </div>
    </div>
  {/if}
{:else if brew.status === 'completed'}
  <section class="invitation">
    ...
    <div class="qr-card"><img src={`/api/v1/brews/${brew.id}/qr.svg`} alt={`QR code to rate ${brew.coffee_name}`} /><strong>Open the camera on your phone</strong><span>Personal sessions stay signed in for 3.5 days.</span></div>
  </section>
{:else}
...
<style>
  .brew-mode { min-height:68vh; display:grid; align-content:center; gap:24px; }
  ...
  @media(max-width:760px){.recipe-display{grid-template-columns:repeat(2,minmax(0,1fr))}.hero-metric{grid-column:span 1; min-height:150px}.recipe-cell{min-height:110px}.invitation{grid-template-columns:1fr}.qr-card{order:-1}.brew-heading{display:block}}
</style>
